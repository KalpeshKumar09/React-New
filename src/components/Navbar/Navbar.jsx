...
const Navbar = () => {
  ...
  let subbase = pathname.split("/")?.[1] || "Dashboard"; // Initialize subbase to 'Dashboard' if empty

  function Lankness(type = null) {
    let classes = "px-4 py-4 rounded inline-flex "; // Fixed typo: Lankness

    if (type === subbase) {
      classes += "bg-violet text-white";
    } else {
      classes += "";
    }
